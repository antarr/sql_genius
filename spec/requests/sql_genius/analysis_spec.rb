...
RSpec.describe("Analysis routes", type: :request) do
  before do
    ...
    # is unit-tested in spec/sql_genius/core/analysis/*_spec.rb;
  ...
  it "GET /sql_genius/duplicate_indexes returns 200 + JSON array" do
    get "/sql_genius/duplicate_indexes"
    expect(last_response).to(be_ok)
    expect(JSON.parse(last_response.body)).to(be_an(Array))
  end

  it "GET /sql_genius/table_sizes returns 200 + JSON array" do
    get "/sql_genius/table_sizes"
    expect(last_response).to(be_ok)
    expect(JSON.parse(last_response.body)).to(be_an(Array))
  end

  it "GET /sql_genius/query_stats returns 200 + JSON array" do
    get "/sql_genius/query_stats"
    expect(last_response).to(be_ok)
    expect(JSON.parse(last_response.body)).to(be_an(Array))
  end

  it "GET /sql_genius/unused_indexes returns 200 + JSON object with indexes + stats context" do
    get "/sql_genius/unused_indexes"
    expect(last_response).to(be_ok)
  ...
  it "GET /sql_genius/server_overview returns 200 + JSON object" do
    get "/sql_genius/server_overview"
    expect(last_response).to(be_ok)
    json = JSON.parse(last_response.body)
    expect(json).to(be_a(Hash))
  ...
  it "GET /sql_genius/slow_queries returns 200 + empty array when Redis not configured" do
    # Redis is not configured in the test env; should return [] not raise.
    get "/sql_genius/slow_queries"
    expect(last_response).to(be_ok)
  ...
  it "does not raise NoMethodError on any analysis route (boot-order regression guard)" do
    ...
    ["duplicate_indexes", "table_sizes", "query_stats", "unused_indexes", "server_overview", "slow_queries"].each do |route|
      expect { get("/sql_genius/#{route}") }.not_to(raise_error)
