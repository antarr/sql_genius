...
RSpec.describe("Query detail routes", type: :request) do
  ...
  describe "GET /sql_genius/queries/:digest" do
    it "returns 200" do
      get "/sql_genius/queries/#{digest}"
  ...
  describe "GET /sql_genius/api/query_history/:digest" do
    context "when stats_history is nil (collection disabled)" do
      before { SqlGenius.stats_history = nil }
      after  { SqlGenius.stats_history = nil }

      it "returns 200 with empty history array" do
        get "/sql_genius/api/query_history/#{digest}"
        expect(last_response.status).to(eq(200))
    ...
    context "when stats_history is set" do
      let(:stats_history) { SqlGenius::Core::Analysis::StatsHistory.new }
      ...
      before do
        ...
        SqlGenius.stats_history = stats_history
      end
