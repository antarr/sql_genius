...
RSpec.describe(SqlGenius::Core::SqlValidator) do
  let(:blocked_tables) { ["sessions", "authentication_tokens"] }
  let(:all_tables) { ["users", "posts", "sessions", "authentication_tokens"] }
  let(:server_info) { SqlGenius::Core::ServerInfo.new(vendor: :mysql, version: "8.0.35") }
  let(:connection) do
    double("connection", tables: all_tables, server_version: server_info, quote_table_name: "`sql_genius_identifier_probe`")
  end
  ...
  describe ".validate" do
    ...
    context "with a PostgreSQL connection" do
      let(:server_info) { SqlGenius::Core::ServerInfo.new(vendor: :postgresql, version: "16.1") }
