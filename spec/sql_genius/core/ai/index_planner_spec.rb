...
RSpec.describe(SqlGenius::Core::Ai::IndexPlanner) do
  ...
  let(:connection) do
    ...
    conn.stub_columns_for("orders", [
      SqlGenius::Core::ColumnDefinition.new(name: "id", sql_type: "bigint", type: :integer, null: false, default: nil, primary_key: true),
      SqlGenius::Core::ColumnDefinition.new(name: "user_id", sql_type: "bigint", type: :integer, null: false, default: nil, primary_key: false),
    ])
    conn.stub_primary_key("users", "id")
    conn.stub_primary_key("orders", "id")
    conn.stub_indexes_for("users", [
      SqlGenius::Core::IndexDefinition.new(name: "idx_users_email", columns: ["email"], unique: true),
      SqlGenius::Core::IndexDefinition.new(name: "idx_users_email_name", columns: ["email", "name"], unique: false),
      SqlGenius::Core::IndexDefinition.new(name: "idx_users_email_dup", columns: ["email"], unique: false),
    ])
    conn.stub_indexes_for("orders", [
      SqlGenius::Core::IndexDefinition.new(name: "idx_orders_user_id", columns: ["user_id"], unique: false),
    ])
