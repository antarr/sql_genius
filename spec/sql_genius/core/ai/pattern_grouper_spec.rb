...
RSpec.describe(SqlGenius::Core::Ai::PatternGrouper) do
  ...
  before do
    ...
    connection.stub_columns_for("orders", [
      SqlGenius::Core::ColumnDefinition.new(name: "id", sql_type: "bigint", type: :integer, null: false, default: nil, primary_key: true),
      SqlGenius::Core::ColumnDefinition.new(name: "status", sql_type: "varchar(50)", type: :string, null: false, default: nil, primary_key: false),
    ])
  ...
  context "when high-cost queries exist" do
    before do
      query_stats_instance = instance_double(SqlGenius::Core::Analysis::QueryStats)
      allow(SqlGenius::Core::Analysis::QueryStats).to(receive(:new).with(connection).and_return(query_stats_instance))
