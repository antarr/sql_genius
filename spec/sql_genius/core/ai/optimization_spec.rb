...
RSpec.describe(SqlGenius::Core::Ai::Optimization) do
  ...
  let(:client) { SqlGenius::Core::Ai::Client.new(ai_config) }

  before do
    connection.stub_tables(["users"])
    connection.stub_columns_for("users", [
      SqlGenius::Core::ColumnDefinition.new(name: "id", type: :integer, sql_type: "bigint", null: false, default: nil, primary_key: true),
      SqlGenius::Core::ColumnDefinition.new(name: "email", type: :string, sql_type: "varchar(255)", null: false, default: nil, primary_key: false),
    ])
    connection.stub_indexes_for("users", [
      SqlGenius::Core::IndexDefinition.new(name: "users_pkey", columns: ["id"], unique: true),
    ])
  end
  ...
  describe "#call" do
    ...
    it "includes both columns and indexes in the schema description" do
      ...
      capturing_config = SqlGenius::Core::Ai::Config.new(
        client: capturing_callable, endpoint: nil, api_key: nil, model: nil, auth_style: :bearer, system_context: nil,
      )
      capturing_client = SqlGenius::Core::Ai::Client.new(capturing_config)
